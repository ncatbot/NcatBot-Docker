...
1. `ubuntu_cn` — 基础 Ubuntu 24.04 中文环境（`ubuntu_cn/Dockerfile`）
2. `ncatbot_env` — Python 3.12 + NapCat 系统依赖（`ncatbot_env/Dockerfile`）
3. `huanyp/ncatbot:latest` — 最终应用镜像（根目录 `Dockerfile`）

**基础镜像（1、2）不打版本 tag，仅使用默认 latest。**
**只有 ncatbot 镜像打版本 tag（`latest` + ncatbot5 PyPI 版本号）。**
...
- 基础镜像在 CI 中构建但不推送，不打版本 tag
- 仅 ncatbot 镜像推送 `latest` + ncatbot5 版本号 tag
...
- Docker tag 不会冲突：重复推送同一 tag 只会更新指向，不会报错
- 基础镜像变更时只需确保 FROM 引用名称一致，无需同步版本号
